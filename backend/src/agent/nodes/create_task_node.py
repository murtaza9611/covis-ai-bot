...
def _cta_scenario_for_task(task: dict) -> str:
    try:
        tt = int(task.get("taskTypeId") or 35)
    except (TypeError, ValueError):
        tt = 35
    if tt == 36:
        return "post_create_bug"
    if tt == 37:
        return "post_create_feature"
...
def _classify_pending_followup(
    ...
    """
    Returns confirm | reject | edit | merge_detail | new_item | status_lookup | chitchat.
    On LLM/JSON failure, default merge_detail to avoid dropping a valid pending draft.
    """
...
def _complete_from_pending(
    ...
    return {
        **_base_state(state),
        ...
        "cta_scenario": _cta_scenario_for_task(task),
    }
    ...
    if early:
        out = {
            **_base_state(state),
            ...
            "response_kind": "text",
            "cta_scenario": "duplicate_detected",
        }
        if dup_token:
...
def _handle_pending_branch(
    ...
    if is_affirmative(q) or llm_is_affirmative(q, llm):
    ...
    if relation == "status_lookup":
        from src.agent.nodes.get_task_node import get_task_node

        return get_task_node({**state, "triage_state": triage_state})

    if relation == "chitchat":
        return {
            **_base_state(state),
            "triage_state": triage_state,
            "pending_task_payload": pending,
